...
de templates **Jinja2** (fourni par Flask) pour injecter une valeur
dans le HTML avant de l'envoyer au navigateur :
`{{ LABEL_TABLE }}` dans `templates/index.html` est remplacé par le
contenu de la variable d'environnement du même nom. En dehors de cette
valeur, les deux mondes ne communiquent que via les appels réseau
`fetch("/create")` décrits ci-dessus.

**Cas particulier de la table des dossiers** : contrairement à
`LABEL_TABLE`, le nom de cette table n'est **pas** une variable
d'environnement — il varie d'un document Grist à l'autre selon la
démarche synchronisée (motif `Demarche_<id>_dossiers`, ex.
`Demarche_128651_dossiers`). Le JavaScript de `templates/index.html`
la détecte donc dynamiquement au chargement, en listant les tables du
