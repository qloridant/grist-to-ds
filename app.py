...
# Ces deux variables sont lues une fois au démarrage du serveur et sont
# réutilisées par la route /create ci-dessous pour appeler l'API DS.
DS_TOKEN = os.getenv("DS_TOKEN")    # jeton d'authentification à l'API Démarches Simplifiées
DS_TARGET = os.getenv("DS_TARGET")  # URL de l'API GraphQL cible (Démarches Simplifiées)
...
def index():
    ...
    # render_template va chercher templates/index.html, remplacer les
    # {{ LABEL_TABLE }} et {{ DOSSIERS_TABLE }} qu'il contient par les
    # valeurs ci-dessus (moteur de templates Jinja2 fourni par Flask),
    # puis renvoyer le HTML final au navigateur.
